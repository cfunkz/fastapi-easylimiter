...
class SlidingWindowStrategy(BaseRedisStrategy):
    """
    Sliding window log rate limiting with atomic ban integration.
    Tracks each request timestamp in a sorted set.
    """
    ...
    local ban_ttl = redis.call('TTL', ban_key)
    if ban_ttl > 0 then
        local oldest = redis.call('ZRANGE', rl_key, 0, 0, 'WITHSCORES')[2]
        local reset = oldest and (tonumber(oldest) + window) or (now + window)
        return {0, 0, 0, ban_ttl, math.floor(reset), now}
    end
    
    -- STEP 2: Clean old entries and check count
    redis.call('ZREMRANGEBYSCORE', rl_key, '-inf', cutoff)
    local count = redis.call('ZCARD', rl_key)
    
    if count < limit then
        -- Allow request
        redis.call('ZADD', rl_key, now, now)
        redis.call('EXPIRE', rl_key, window + 60)
        local oldest = redis.call('ZRANGE', rl_key, 0, 0, 'WITHSCORES')[2] or now
        local reset = tonumber(oldest) + window
        return {1, limit - count - 1, math.floor(reset), 0, math.floor(reset), now}
    else
        ...
        if offenses >= ban_after then
            ...
            local duration = math.min(initial_ban * (2 ^ (level - 1)), max_ban)
            redis.call('SET', ban_key, '1', 'EX', duration)
            return {0, 0, 0, duration, now + window, now}
        end
        
        local oldest = redis.call('ZRANGE', rl_key, 0, 0, 'WITHSCORES')[2] or (now - window)
        local reset = tonumber(oldest) + window
        return {0, 0, math.floor(reset), 0, math.floor(reset), now}
    end
    """
